 ...
 * @param[in] flags  Attribiutes of the new directory
 *
...
PUBLIC int do_chdir()
{
	...
	if (ino->i_mode != I_DIRECTORY) {
		put_inode(ino);
		return -ENOTDIR;
	}
	put_inode(current->pwd);
	current->pwd = ino;
	return 0;
}
